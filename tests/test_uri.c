...
{
    {
        ...
        assert(-EFAULT == uri_userinfo_set(NULL, "u"));
        assert(-EFAULT == uri_host_set(NULL, "h"));
        ...
        assert(-EINVAL == uri_new("scheme:%a", &u));
        assert(-EINVAL == uri_new("scheme:%a ", &u));
        assert(-EINVAL == uri_new("scheme:%ag", &u));
        assert(-EINVAL == uri_new("scheme:% 2e", &u));
        ...
        assert(      0 == uri_userinfo_set(u, "u:p"));
        assert(!strcmp(uri_path(u), "/path here"));
        assert(0 == uri_str(u, &str));
        assert(!strcmp(str, "s://u:p@h%2Fh/path%20here"));
        free(str);
        uri_delete(u);

        assert(      0 == uri_new("", &u));
        assert(      0 == uri_host_set(u, "h"));
        assert(      0 == uri_str(u, &str));
        assert(      0 == strcmp(str, "//h"));
        free(str);
        ...
        // Note that uri_str() uses percent-encoding to conform to RFC 3986.
        assert(0 == uri_str(u, &str));
        assert(!strcmp(str, "s://h%2Fh/path%252fhere"));
        free(str);
        uri_delete(u);

        // Caller has escaped the slash "%2f"; the percent itself must be percent-encoded.
        assert(0 == uri_new("s://h%2Fh/path%252Fhere", &u));
        assert(!strcmp(uri_path(u), "/path%2Fhere"));
        assert(0 == uri_str(u, &str));
        assert(!strcmp(str, "s://h%2Fh/path%252Fhere"));
        free(str);
        ...
        assert(0 == uri_new("s://h%2fh", &u));
        // Caller has escaped the slash "%2f"; the percent itself must be percent-encoded.
        uri_set(u, "s://h%2Fh/path%252fhere");
        assert(!strcmp(uri_path(u), "/path%2fhere"));
        assert(0 == uri_str(u, &str));
        assert(!strcmp(str, "s://h%2Fh/path%252fhere"));
        free(str);
    ...
    test_modify("/p",         "o",   "/o");
    test_modify("/p/",        "o",   "/p/o");
    test_modify("//h/p",      "o",   "//h/o");
    test_modify("//h/p/",     "o",   "//h/p/o");
